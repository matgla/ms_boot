...
{
public:
    using OutType = eul::function<void(std::string_view data), sizeof(void*)>;
    IOPrinter(const OutType& out);

    template <typename T>
    IOPrinter& operator<<(const T& t)
    {
        write(t);
        return *this;
    }

    IOPrinter& operator<<(bool b);
    IOPrinter& operator<<(const EndlineTag);
    IOPrinter& operator<<(const logging_flags::base base);
    IOPrinter& operator<<(const logging_flags::boolalpha b);
