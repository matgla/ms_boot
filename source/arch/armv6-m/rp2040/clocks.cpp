...
void pll_init(pll_hw_t *pll, uint32_t refdiv, uint32_t vco_freq,
...
bool clock_configure(clock_index clk_index, uint32_t src, uint32_t auxsrc,
                     uint64_t src_freq, uint32_t freq) {
  if (freq > src_freq) {
    return false;
  }
  ...
  if (has_glitchless_mux(clk_index) &&
    ...
    if (configured_freq[clk_index] > 0) {
      uint32_t delay =
          configured_freq[clk_sys] / configured_freq[clk_index] + 1;
      delay_clocks(delay);
    }
  }

  write_bits(&clk->ctrl, (auxsrc << CLOCKS_CLK_SYS_CTRL_AUXSRC_LSB),
             CLOCKS_CLK_SYS_CTRL_AUXSRC_BITS);

  if (has_glitchless_mux(clk_index)) {
    write_bits(&clk->ctrl, src << CLOCKS_CLK_REF_CTRL_SRC_LSB,
...
void clocks_init() {
  ...
  clock_configure(clk_ref, CLOCKS_CLK_REF_CTRL_SRC_VALUE_XOSC_CLKSRC, 0, 12_MHz,
                  12_MHz);

  clock_configure(clk_sys, CLOCKS_CLK_SYS_CTRL_SRC_VALUE_CLKSRC_CLK_SYS_AUX,
                  CLOCKS_CLK_SYS_CTRL_AUXSRC_VALUE_CLKSRC_PLL_SYS, 125_MHz,
                  125_MHz);
